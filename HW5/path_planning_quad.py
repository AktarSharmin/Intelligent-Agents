...
def create_source():
    ...
    sim.setObjectPosition(source_handle, -1, [-1, -1.5, 0])
...
def create_destination():
    ...
    sim.setObjectPosition(destination_handle, -1, [1.5, 1.5, 0])
...
def create_quad():
    ...
    sim.setObjectPosition(quad_handle,-1,[-1, -1.5, 0])
    quad_pos=sim.getObjectPosition(quad_handle,-1)
    return quad_handle,quad_pos
...
def create_trajectory(path):
    ...
    trajectory_handle = sim.createPath(control_points, 8, total_length  // dt, 1)
...
def sysCall_init():
    ...
    quad_handle,quad_pos = create_quad()
