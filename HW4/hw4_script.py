...
def create_source():
    ...
    sim.setObjectPosition(source_handle, -1, [-1, -1.5, 0.05])
...
def create_destination():
    ...
    sim.setObjectPosition(destination_handle, -1, [1.5, 1.5, 0.05])
...
def create_obstacles_boundary(obstacles):
    ...
    for obstacle in obstacles:
        ...
        sim.setObjectPosition(obstacle_handle, -1, [obstacle[0], obstacle[1], 0])
        sim.setObjectColor(obstacle_handle,0,0, [0, 0, 0])
...
def create_path(path):
    ...
    # Create a new drawing object for the path
    line_handle = sim.addDrawingObject(sim.drawing_lines, 6, 0.0, -1, 9999, [1,0,0])

    # Iterate over pairs of consecutive points in the path and add line segments to the drawing object
    for i in range(len(path)-1):
        sim.addDrawingObjectItem(line_handle, [path[i][0], path[i][1], 0, path[i+1][0], path[i+1][1], 0])
...
def sysCall_init():
    ...
    #create the path
    create_path(np.stack([scaled_rx, scaled_ry],axis=1))
